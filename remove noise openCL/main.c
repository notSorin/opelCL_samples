...
void writeBMP(float *imageFLOAT, char *file_name, char header[54], int width, int height)
{
	...
	for (i=0;i<width*height;i++){
		image[3*i]   = (unsigned char)imageFLOAT[i]; //R 
		image[3*i+1] = (unsigned char)imageFLOAT[i]; //G
		image[3*i+2] = (unsigned char)imageFLOAT[i]; //B
	}
...
float *RGB2BW(unsigned char *imageUCHAR, int width, int height)
{
	...
	for (i=0; i<height; i++)
		for (j=0; j<width; j++)
		{
			...
			imageBW[i*width+j] = (float)(0.2989 * R + 0.5870 * G + 0.1140 * B);
		}
...
void remove_noise(float *im, float *image_out, float thredshold, int window_size, int height, int width)
{
	...
	float median;
	int ws2 = (window_size - 1) >> 1; //ws2 = (3 - 1) / 2 = 1
...
int main(int argc, char **argv) {
	...
	switch (argv[3][0]) {
		case 'c':
			t0 = get_time();
			remove_noise(imageBW, imageOUT, 0.1f, 3, height, width);
			t1 = get_time();
			printf("CPU Exection time %f s.\n", t1-t0);
			break;
		case 'g':
//			t0 = get_time();
			remove_noiseOCL(imageBW, imageOUT, 0.1f, 3, height, width);
//			t1 = get_time();
